...
#include "../wfb-ng/src/tx.hpp"
#include "../DualDevourer/src/RtlUsbAdapter.h"
#include "../DualDevourer/src/WiFiDriver.h"
...
void RawSocketTransmitter::inject_packet(const uint8_t *buf, size_t size)
{
    ...
    uint8_t *buffer = new uint8_t[radiotap_header.header.size() +sizeof(ieee_hdr)+size ](0);
    printf("total size:%d\n",radiotap_header.header.size() +sizeof(ieee_hdr)+size );

    memcpy(buffer,(uint8_t*)(&radiotap_header.header[0]),radiotap_header.header.size());
    memcpy(buffer+ radiotap_header.header.size(),(uint8_t*)(ieee_hdr),sizeof(ieee_hdr));
    memcpy(buffer + radiotap_header.header.size() + sizeof(ieee_hdr),(uint8_t*)buf,size);
...
int open_control_fd(int control_port)
{
  ...
  libusb_device_handle *handle;
  libusb_device *device;
  struct libusb_device_descriptor desc;      throw runtime_error(string_format("Unable to get socket info: %s", strerror(errno)));
        }
...
int main(int argc, char * const *argv)
{
	...
	case 'E':
        ...
        case 'X':
            ...
            break;
        case 'c':
            channel = static_cast<uint8_t>(atoi(optarg));
            break;
        case 'I':
            tx_mode = INJECTOR;
    ...
    {
        if(device_id == -1){
            fprintf(stderr, "No usb device found;Please use termux-usb to load usb device;");
            return 1;
        }
        ...
        rc = libusb_wrap_sys_device(context, (intptr_t) device_id, &handle);
        ...
        rtlDevice = wifi_driver.CreateRtlDevice(handle);
